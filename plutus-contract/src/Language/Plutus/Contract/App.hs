...
printTrace :: StatefulContract () -> Wallet -> ContractTrace EmulatorAction () () -> IO ()
printTrace con wllt ctr = void (foldM go (initialResponse con) events) where
    events = Map.findWithDefault [] wllt $ execTrace (State.lower con) ctr
    go previous evt = do
